...
if __name__ == "__main__":
    ...
    critical_temperatures = pd.DataFrame(
        columns=["mu", "T_C", "beta_C"], index=range(len(results.keys()))
    )
    ...
    for index, (mu, results) in enumerate(results.items()):
        ...
        linear_reg = stats.linregress(
            linear_T_interval["temperature"], linear_T_interval["order_parameter"]
        )

        ax.plot(
            linear_T_interval.loc[:, "temperature"],
            linear_T_interval.loc[:, "order_parameter"],
            "x--",
        )
        ax.plot(
            linear_T_interval.loc[:, "temperature"],
